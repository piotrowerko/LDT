...
dlugosc=length(A);
analiza=fft(A,ile_k); %analiza=fft(dane,liczba_próbek);
widmo_am=abs(analiza);
figure(1);
plot(linspace(0,czestotliwosc,length(abs(fft(A)))),abs(fft(A)),'r'); %plot(linspace(start_wykresu,czestosliwosc_probkowania,dlugosc_wektora),abs(fft(wektor_danych,ilo??_probek_do_analizy)),kolor_wykresu);
...
hold on;
plot (t,A);
title(' Hilbert Transform for transient signal');
ylabel('Displacment (mm), amplitude');
xlabel('Time (s)');
hold on;
t=time(k:k2);
...
plot(cfun2,'r');
xlabel('czas (s)');
ylabel('Log Amplitude of Vibrations');
legend({'Log representation of Hilbert transform','Liner model roubst fit'});
